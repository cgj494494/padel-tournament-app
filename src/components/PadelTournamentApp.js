                ...
                <div className="mb-8">
                    ...
                    <PlayerSelectionView
                        onPlayersSelected={(selectedPlayers) => {
                            onCreateTournament(pendingTournamentName, selectedPlayers);
                            setCreationStep('naming');
                            setPendingTournamentName('');
                        }}
                        minPlayers={5}
...
const PadelTournamentApp = () => {
    ...
    // 5. Fix the createNewTournament function
    const createNewTournament = (name, selectedPlayers = null) => {
        ...
        if (!selectedPlayers) {
            ...
            // Use default matches with the proper structure
            newMatches = [
                {
                    round: 1,
                    time: "11:15",
                ...
                // Include all other default matches
                {
                    round: 2,
                    time: "11:28",
                    court1: {
                ...
                // Add the remaining default matches here...
            ];
        } else {
            // Convert selected players to the format used by the tournament
            newPlayers = selectedPlayers.map((player) => ({
                id: player.id,
            ...
            newMatches = generateTournamentSchedule(newPlayers);
        }

        console.log("New matches created:", newMatches);

        // Update state in a way that ensures synchronization
        setPlayers(newPlayers);
    ...
    // 2. Add safety checks to getGames function
    const getGames = (court, team) => {
        ...
        try {
            ...
            return games !== null && games !== undefined ? games : '-';
        } catch (error) {
    ...
    // 4. Add safety checks to getTournamentPoints function
    const getTournamentPoints = (court, team) => {
        if (!currentMatch) return 0;

        try {
            if (court === 1 && currentMatch.court1) {
                const [pointsA, pointsB] = calculatePoints(
            ...
            } else if (court === 2 && currentMatch.court2) {
                const [pointsA, pointsB] = calculatePoints(
            ...
            console.error("Error in getTournamentPoints:", error);
            return 0;
        }
    };
    ...
    // 6. Fix the generateFlexibleSchedule function
    const generateFlexibleSchedule = (players) => {
        ...
        const totalRounds = Math.max(totalPlayers - 1, 1); // At least 1 round
        ...
        for (let round = 1; round <= totalRounds; round++) {
            ...
            const notPlaying = playingThisRound.slice(playersPerRound)[0] || playerIds[0];

            // Ensure we have enough players for teams
            while (playersOnCourt.length < 8) {
                playersOnCourt.push(playerIds[0]); // Use first player as a placeholder
            }
            ...
            const matchRound = {
                ...
                court1: {
                    teamA: playersOnCourt.length >= 2 ? [playersOnCourt[0], playersOnCourt[1]] : [playerIds[0], playerIds[0]],
                    teamB: playersOnCourt.length >= 4 ? [playersOnCourt[2], playersOnCourt[3]] : [playerIds[0], playerIds[0]],
                    gamesA: null,
                    gamesB: null,
                    scoreA: null,
                    scoreB: null
                ...
                court2: {
                    teamA: playersOnCourt.length >= 6 ? [playersOnCourt[4], playersOnCourt[5]] : [playerIds[0], playerIds[0]],
                    teamB: playersOnCourt.length >= 8 ? [playersOnCourt[6], playersOnCourt[7]] : [playerIds[0], playerIds[0]],
                    gamesA: null,
                    gamesB: null,
                    scoreA: null,
                    scoreB: null
