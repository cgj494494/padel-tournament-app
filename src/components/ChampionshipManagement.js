...
const ChampionshipManagement = ({ saveLastUsed }) => {
    ...
    const handleEditMatchClick = (match) => {
        ...
        setEditScores({
            // Check both possible locations for scores
            teamA: (match.score?.teamA !== undefined)
                ? match.score.teamA.toString()
                : match.gamesA?.toString() || '0',
            teamB: (match.score?.teamB !== undefined)
                ? match.score.teamB.toString()
                : match.gamesB?.toString() || '0'
        });

        // Check both possible completion status properties
        setEditComplete(
            (match.complete !== undefined) ? match.complete : (match.isComplete !== false)
        );

        // NEW: Also capture any existing point details
        if (match.pointDetails) {
            setEditPointDetails(match.pointDetails);
        } else {
            setEditPointDetails(null);
        }
    ...
    const handleSaveEditedMatch = () => {
        ...
        // NEW: Check if this is a tied score that needs point details
        // We'd show the game points dialog for tied scores, regardless of ambiguity
        if (gamesA === gamesB) {
            // Store the editing match ID in session storage
            sessionStorage.setItem('editingMatchId', editingMatch.id.toString());

            // Set temp scores for the game points dialog
            setTempGameScores({ gamesA, gamesB });

            // If we already have point details, preload them
            if (editPointDetails) {
                if (editPointDetails.type === 'regular') {
                    setPointInputType('tennis');
                    setGamePoints({
                        teamA: editPointDetails.teamAPoints,
                        teamB: editPointDetails.teamBPoints
                    });
                    setTiebreakPoints({ teamA: '', teamB: '' });
                } else {
                    setPointInputType('numeric');
            ...
            // Close edit dialog and show game points dialog
            setShowEditDialog(false);
            setShowGamePointDialog(true);
        }
        // Handle ambiguous score that might need tiebreak handling
        else if (isAmbiguousScore(gamesA, gamesB) && editComplete) {
            ...
            // NEW: Pass the point details (which should be null for non-tied scores)
            saveUpdatedMatch(editingMatch.id, gamesA, gamesB, editComplete, editingMatch.date, null);
        }
    };
    ...
    const saveUpdatedMatch = (matchId, gamesA, gamesB, isComplete, matchDate, pointDetails = null) => {
        // Calculate points based on new scores
        // If there are point details with advantage, add a bonus point
        let [pointsA, pointsB] = calculateCJPoints(gamesA, gamesB, isComplete);

        // Add bonus point for team with advantage in points (if applicable)
        if (pointDetails) {
            if (pointDetails.teamAAdvantage) {
                pointsA += 1; // Bonus point for team A
            } else if (pointDetails.teamBAdvantage) {
                pointsB += 1; // Bonus point for team B
            }
        }
        ...
        const updatedMatches = updatedChampionship.matches.map(match => {
            if (match.id === matchId) {
                ...
                // Add point details if provided
                if (pointDetails) {
                    updatedMatch.pointDetails = pointDetails;
                } else if (gamesA === gamesB) {
                    // If score is tied but no point details provided,
                    // keep existing ones if they exist
                    if (match.pointDetails) {
                        updatedMatch.pointDetails = match.pointDetails;
                    }
                } else {
                    // For non-tied scores, remove any existing point details
                    delete updatedMatch.pointDetails;
                }
        ...
        // Update the championship with the new matches
        ...
        <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-100">
            ...
            <div className="pt-20 pb-32 px-2">
                <div className="max-w-6xl mx-auto">
                    ...
                    {/* Edit Match Dialog - Add near other modals in the return statement */}
                            ...
                            <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-xl">
                                ...
                                <button
                                    onClick={() => {
                                        ...
                                        setTimeout(() => {
                                            setShowDeleteConfirmation(true); // Then show delete confirmation
...
if (view === 'session') {
    return (
        <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-100">
            ...
            {/* 
   PLACEMENT INSTRUCTION: Add this Game Points Dialog at the END of your component's return statement
   It should be AFTER all other content but BEFORE the final closing tag of your component
   It should be a SIBLING to other dialogs, not nested inside them
   Look for a section with your other modal dialogs and add this at the same level
*/}
            {showGamePointDialog && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-xl p-6 max-w-lg w-full">
                                ...
                                <div>
                                    ...
                                    <div className="mt-4">
                                        ...
                                        <input
                                            ...
                                            className={`
                                    w-full border border-gray-300 rounded-lg px-3 py-2 text-center
                                    ${pointInputType === 'tennis' ? 'bg-gray-100 opacity-50 cursor-not-allowed' : ''}
                                `}
                                        />
                                    </div>
                                </div>
                        ...
                        <div className="mb-6 border-t border-b border-gray-200 py-4">
                            {pointInputType ? (
                                <div className="text-center">
                                    <p className="font-medium mb-1">Game Point Status:</p>

                                    {pointInputType === 'tennis' ? (
                                        <p className="text-lg">
                                            {gamePoints.teamA} - {gamePoints.teamB}
                                            {gamePoints.teamA === 'AD' && <span className="text-green-600 ml-2">(Team A Advantage)</span>}
                                            {gamePoints.teamB === 'AD' && <span className="text-green-600 ml-2">(Team B Advantage)</span>}
                                        </p>
                                    ) : (
                                        <p className="text-lg">
                                            Tiebreak: {tiebreakPoints.teamA || '0'} - {tiebreakPoints.teamB || '0'}
                                        </p>
                                    )}
                                </div>
                            ) : (
                                <p className="text-center text-gray-500">No point status selected yet.</p>
                            )}
                        </div>
                            ...
                            <div>
                                ...
                                <button
                                    onClick={() => {
                                        ...
                                        // Determine if match is complete based on score
                                        // For ties, all sets are assumed complete
                                        const isComplete = gamesA === gamesB ? true : detectComplete(gamesA, gamesB);

                                        // PLACEMENT INSTRUCTION: This calls a function that will be added in the next section
                                        saveMatchWithPointDetails(gamesA, gamesB, isComplete, pointDetails);

                                        // Close dialog
                                        setShowGamePointDialog(false);
                                    }}
                                    disabled={!pointInputType}
                                    className={`
                            px-4 py-2 rounded-lg 
                            ${pointInputType ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-gray-300 cursor-not-allowed'}
                        `}
                                >
                                    Save
                                </button>
